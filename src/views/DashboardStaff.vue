            ...
            <div class="sm:flex sm:items-start">
              <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                ...
                <form @submit.prevent="saveStaff" class="space-y-4">
                  ...
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Qualifications (Optional)</label>
                    <textarea
                        v-model="staffForm.qualifications"
                        rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                ...
                <div v-if="selectedStaff" class="space-y-3">
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p class="text-sm text-gray-900">{{ selectedStaff.first_name }} {{ selectedStaff.last_name }}</p>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Email:</span>
                      <p class="text-sm text-gray-900">{{ selectedStaff.email }}</p>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Phone:</span>
                      <p class="text-sm text-gray-900">{{ selectedStaff.phone_number }}</p>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Role:</span>
                      <p class="text-sm text-gray-900">{{ formatRole(selectedStaff.role) }}</p>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Department:</span>
                      <p class="text-sm text-gray-900">{{ selectedStaff.department || 'N/A' }}</p>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Status:</span>
                      <span :class="getStatusClasses(selectedStaff.status)">{{ formatStatus(selectedStaff.status) }}</span>
                    </div>
                    <div>
                      <span class="text-sm font-medium text-gray-500">Hire Date:</span>
                      <p class="text-sm text-gray-900">{{ formatDate(selectedStaff.hire_date) }}</p>
                    </div>
                    <div v-if="selectedStaff.salary">
                      <span class="text-sm font-medium text-gray-500">Salary:</span>
                      <p class="text-sm text-gray-900">${{ Number(selectedStaff.salary).toLocaleString() }}</p>
                    </div>
                  </div>
                  <div v-if="selectedStaff.qualifications">
                    <span class="text-sm font-medium text-gray-500">Qualifications:</span>
                    <p class="text-sm text-gray-900 mt-1">{{ selectedStaff.qualifications }}</p>
                  </div>
                </div>
...
const saveStaff = async () => {
  ...
  try {
    const url = showEditModal.value
        ? `/api/staff/${selectedStaff.value.id}`
        : '/api/staff'
    ...
    if (response.ok) {
      ...
      console.error('Failed to save staff:', errorData)
