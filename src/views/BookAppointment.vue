...
const loadDoctors = async (departmentId) => {
  ...
  try {
    ...
    doctors.value = departmentDoctors.map(doctor => ({
      value: doctor.id,
      label: `Dr. ${doctor.first_name}  ${doctor.last_name} - ${doctor.specialization || 'General'}`
    }))
  } else {
            ...
            <form @submit.prevent="submitAppointment" class="space-y-6">
              <div class="border-b border-gray-200 pb-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <BaseInput id="patient_name" :label="t('appointment.fullName')" v-model="appointmentForm.patient_name"
                    :placeholder="t('appointment.placeholders.fullName')" :error="errors.patient_name" :required="true"
                    autocomplete="name" @focus="clearError('patient_name')" />
                  <BaseInput id="phone_number" :label="t('appointment.phoneNumber')"
                    v-model="appointmentForm.phone_number" type="tel"
                    :placeholder="t('appointment.placeholders.phoneNumber')" :error="errors.phone_number"
                    :required="true" autocomplete="tel" @focus="clearError('phone_number')" />
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 gap-6">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <BaseSelect id="department_id" :label="t('appointment.department')"
                    v-model="appointmentForm.department_id"
                    :placeholder="t('appointment.placeholders.selectDepartment')" :options="departmentOptions"
                    :error="errors.department_id" :required="true" @update:modelValue="onDepartmentChange" />
                  <BaseSelect id="service_id" :label="t('appointment.service')" v-model="appointmentForm.service_id"
                    :placeholder="t('appointment.placeholders.selectService')" :options="serviceOptions"
                    :error="errors.service_id" :required="true" @update:modelValue="clearError('service_id')" />
                </div>

                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                  <BaseSelect id="doctor_id" :label="t('appointment.doctor')" :required="true"
                    v-model="appointmentForm.doctor_id" :placeholder="t('appointment.placeholders.selectDoctor')"
                    :options="doctorOptions" :error="errors.doctor_id" @update:modelValue="onDoctorChange" />
                  <BaseInput id="appointment_date" :label="t('appointment.preferredDate')"
                    v-model="appointmentForm.appointment_date" type="date" :min="minDate"
                    :error="errors.appointment_date" :required="true" @change="onDateChange" />
                </div>

                <!-- Time Slot Selection -->
                <div v-if="appointmentForm.doctor_id && appointmentForm.appointment_date">
                  <BaseSelect id="appointment_time" label="Available Time Slots"
                    v-model="appointmentForm.appointment_time" placeholder="Select an available time slot"
                    :options="timeSlotOptions" :error="errors.appointment_time" :required="true"
                    :loading="loadingTimeSlots" @update:modelValue="clearError('appointment_time')" />

                  <!-- Show message if no slots available -->
                  <div
                    v-if="!loadingTimeSlots && availableTimeSlots.length === 0 && appointmentForm.doctor_id && appointmentForm.appointment_date"
                    class="mt-2 text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-md p-3">
                    <div class="flex">
                      <svg class="h-4 w-4 text-amber-400 mt-0.5 mr-2" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
