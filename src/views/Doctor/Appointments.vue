      ...
      <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-medium text-gray-900">
            {{ tabs.find(t => t.id === activeTab)?.name }} Appointments
          </h3>
        </div>
          ...
          <div class="mt-3">
            ...
            <div class="mb-4">
              <p class="text-sm text-gray-600 mb-2">
                Patient: <span class="font-medium">{{ selectedAppointment?.patient.name }}</span>
              </p>
              <p class="text-sm text-gray-600 mb-4">
                Current: <span class="font-medium">{{ formatDateTime(selectedAppointment?.datetime) }}</span>
              </p>
            </div>
            ...
            <form @submit.prevent="rescheduleAppointment" class="space-y-4">
                ...
                <div class="flex">
                  ...
                  <div class="ml-3">
                    <p class="text-sm text-yellow-800">
                      Warning: This time slot conflicts with an existing appointment.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex justify-end space-x-3 pt-4">
                ...
                <button
                    type="submit"
                    :disabled="!rescheduleForm.date || !rescheduleForm.time || !isValidTime"
...
const timeConflict = computed(() => {
  ...
  return appointments.value.some(apt => {
    ...
    return Math.abs(newDateTime.getTime() - existingDateTime.getTime()) < 30 * 60 * 1000 // 30 minutes buffer
  })
})
...
const rescheduleAppointment = async () => {
  try {
    ...
    if (appointmentIndex !== -1) {
      ...
      // Optional: Add reschedule history
